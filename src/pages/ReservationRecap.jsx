...
export default function ReservationRecap() {
  ...
  if (selectedSeats.length === 0) {
    return (
      <div
        className="reservation-recap min-h-screen flex items-center justify-center p-6"
  ...
  return (
    <div
      className="reservation-recap min-h-screen p-6"
      style={{ backgroundImage: "url('/page_fond_evenements.png')" }}
    >
      <div className="max-w-4xl mx-auto space-y-8 bg-white/90 backdrop-blur-sm p-6 rounded-lg shadow-lg">
          ...
          <ul className="list-disc list-inside text-gray-700">
            {selectedSeats.map((seat, i) => {
              ...
              if (seat.id !== "PASS" && seat.id !== "GEN" && seat.id !== "VIP") {
                billetLabel = `Siège ${seat.id} – ${category?.name || seat.category}`;
              } else {
